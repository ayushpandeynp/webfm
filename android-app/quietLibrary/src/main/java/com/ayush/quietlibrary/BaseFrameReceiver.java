...
public abstract class BaseFrameReceiver {
    ...
    private native long nativeRecv(byte[] frame, long offset, long length, short[] audioBuf) throws IOException;
    ...
    public long receive(byte[] frame, short[] audioBuf) throws IOException {
        return this.nativeRecv(frame, 0, frame.length, audioBuf);
    }

    public long receive(byte[] frame, long length, short[] audioBuf) throws IOException {
        return this.nativeRecv(frame, 0, length, audioBuf);
    }

    public long receive(byte[] frame, long offset, long length, short[] audioBuf) throws IOException {
        return this.nativeRecv(frame, offset, length, audioBuf);
    }
